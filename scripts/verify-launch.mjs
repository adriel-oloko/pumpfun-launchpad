...
if (!mintStr) {
  console.error("usage: node scripts/verify-launch.mjs --mint <mintAddress> [--rpc <url>]");
  process.exit(1);
}
...
async function main() {
  ...
  const keypairPath =
    process.env.SOLANA_KEYPAIR ?? path.join(os.homedir(), ".config", "solana", "devnet.json");
  ...
  console.log(`explorer: ${EXPLORER}/address/${mint.toBase58()}?cluster=devnet`);
  ...
  try {
    const curveData = await program.account.curveStateAccount.fetch(curveState);
    console.log(`curve state: solReserve=${curveData.solReserve.toString()} tokenReserve=${curveData.tokenReserve.toString()} supplyOut=${curveData.supplyOut.toString()}`);
    console.log(`price      : ${(Number(curveData.solReserve.toString()) / Number(curveData.tokenReserve.toString())).toFixed(6)} lamports/token`);
  } catch (e) {
    console.log(`curve state: read failed (${e instanceof Error ? e.message : e})`);
  }
  ...
  try {
    ...
    for (const h of largest.value.slice(0, 8)) {
      console.log(`   ${h.address.toBase58()}  ${(Number(h.amount) / 1e6).toFixed(4)} tokens`);
    }
  } catch (e) {
    console.log(`holders    : read failed (${e instanceof Error ? e.message : e})`);
  }
