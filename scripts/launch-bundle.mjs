...
//   --rpc <url>          devnet RPC (default https://devnet.helius-rpc.com/?api-key=6fd05d57-a073-4cc6-8b5b-4314a652e487)
...
async function main() {
    ...
    if (!fs.existsSync(walletsFile)) {
        console.error(
            `no wallet file at ${walletsFile}; run: node scripts/gen-wallets.mjs 6`
        )
        process.exit(1)
    }
    ...
    if (perWalletRaw) {
        ...
        if (solIns.length !== wallets.length) {
            console.error(
                `--per-wallet has ${solIns.length} values but ${wallets.length} wallets`
            )
            process.exit(1)
        }
    } else {
    ...
    log(
        `creator  : ${creator.publicKey.toBase58()}  (${fmtLamports(creatorBalance)} SOL)`
    )
    log(`nonce    : ${nonce}`)
    ...
    log(
        `migration: auto_migrate=${autoMigrate} lock_lp=${lockLp} (M3 flags passed to create)`
    )
    log(`wallets  : ${wallets.length}`)
    for (let i = 0; i < wallets.length; i++) {
        log(
            `   ${wallets[i].label.padEnd(8)} ${wallets[i].kp.publicKey.toBase58()}  buys ${solIns[i].toFixed(4)} SOL`
        )
    }
    log(
    ...
    if (
        doFund &&
        creatorBalance < Number(totalFund) + 0.02 * LAMPORTS_PER_SOL
    ) {
    ...
    if (tier === '1' || tier === 'both') {
        log(
            '---------------- TIER 1: normal devnet send (no Jito) ----------------'
        )
        ...
        for (const bt of seq1.buyTxs) {
            log(
                `   ${bt.wallets.length} wallets, ${bt.signedSize} bytes (budget 1222)`
            )
        }
        ...
        const sent = await lib.sendSequentially(connection, seq1, {
            onSignature: (label, sig) =>
                log(
                    `[${label.padEnd(6)}] ${sig}  ${EXPLORER}/tx/${sig}?cluster=devnet`
                ),
        })
        ...
        const balances = []
        for (let i = 0; i < wallets.length; i++) {
            const bal = await lib.walletTokenBalance(
                connection,
                wallets[i].kp.publicKey,
                pda.mint
            )
            balances.push(bal)
            log(
        ...
        try {
            ...
            log(
                'holders (getTokenLargestAccounts on-chain): rate-limited on the public'
            )
            log(
                '   devnet RPC; the known-wallet roster above is the authoritative count.'
            )
        }
        const curve = await program.account.curveStateAccount.fetch(
            pda.curveState
        )
        log(
        ...
        log(
            `token      : ${EXPLORER}/address/${pda.mint.toBase58()}?cluster=devnet`
        )
        log(
            `sent txs   : ${sent.length} (${sent.map((s) => s.label).join(', ')})`
        )
        log()
    }
    ...
    if (tier === '2' || tier === 'both') {
        log(
            '---------------- TIER 2: Jito bundle (assembly + sim + submit) ----------------'
        )
        const nonce2 = nonceArg ? BigInt(nonceArg) + 1n : BigInt(Date.now() + 1)
        const pda2 = lib.derivePdas(
            program.programId,
            creator.publicKey,
            nonce2
        )
        log(
            `fresh launch for the bundle: nonce ${nonce2}, mint ${pda2.mint.toBase58()}`
        )
        ...
        try {
            tipAccounts = await lib.withTimeout(
                jito.getTipAccounts(),
                10_000,
        ...
        if (unreachable) {
            ...
            log(
                '   this matches the known devnet reality: the devnet block engine'
            )
            log(
                '   host does not resolve, so devnet bundles cannot be submitted or land.'
            )
            log(
                '   the construction stays fully testable without Jito (Tier 1 proved it).'
            )
        } else {
            log(
                `jito endpoint ${bundleEndpoint}: reachable (${tipAccounts.length} tip accounts)`
            )
        }
        ...
        let liveTips = []
        try {
            const v = await lib.withTimeout(
                mainnetJito.verifyKnownTipAccounts(),
                10_000,
                'verifyKnownTipAccounts timed out'
            )
            liveTips = v.live
            ...
            log(
                `   note       : the prompt's first address is missing a trailing "5";`
            )
            log(
                `               the canonical account is ${lib.KNOWN_TIP_ACCOUNTS[0]}`
            )
        } catch (e) {
            log(
                `tip account verification failed: ${e instanceof Error ? e.message : e}`
            )
        }
        const tipAccount = new PublicKey(
            liveTips.length > 0
                ? liveTips[Math.floor(Math.random() * liveTips.length)]
                : lib.KNOWN_TIP_ACCOUNTS[0]
        )
        ...
        log(
            `bundle assembled: ${assembled.base64.length} txs, tip ${tipLamports} lamports -> ${tipAccount.toBase58()}`
        )
        ...
        for (const s of sims)
            log(`   ${s.label.padEnd(14)} ${s.unitsConsumed} CU, ok`)

        if (unreachable) {
            // Encoding validation against the reachable mainnet endpoint.
            log()
            log(
                '=== sendBundle encoding validation (mainnet endpoint, devnet bundle) ==='
            )
            log(
                '   sending the devnet bundle to the reachable mainnet block engine.'
            )
            log(
                '   a rejection is EXPECTED (devnet blockhash / program do not exist on'
            )
            log(
                '   mainnet); a parse error instead would prove broken base64 encoding.'
            )
            try {
                ...
                log(
                    `   mainnet accepted the devnet bundle (id ${bundleId}); polling status...`
                )
                ...
                log(
                    `   mainnet bundle status: ${JSON.stringify(finalStatus ?? 'no final status')}`
                )
            } catch (e) {
            ...
            log(
                '   => the request was parsed and the base64 decoded (a malformed bundle'
            )
            log(
                '      would fail JSON-RPC parsing before any chain check); the rejection is'
            )
            log(
                '      chain-level (devnet blockhash / program do not exist on mainnet),'
            )
            log('      exactly as expected. No tip moves on a rejected bundle.')
            log()
            log(
                'TIER 2 RESULT: bundle assembled, base64-encoded, tip-carrying, and fully'
            )
            log(
                'simulated; submission to the devnet block engine is impossible (host does'
            )
            log(
                'not resolve). No fabricated bundle-landing claim: devnet bundles cannot land.'
            )
        } else {
