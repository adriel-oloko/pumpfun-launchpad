...
const { Connection, Keypair, LAMPORTS_PER_SOL, PublicKey, Transaction } = (await import("@solana/web3.js"));
...
async function main() {
  ...
  console.log(`creator ${creator.publicKey.toBase58()}  ${(creatorBal / LAMPORTS_PER_SOL).toFixed(4)} SOL`);
  console.log(`rpc     ${rpcUrl}`);
  const needed = BigInt(Math.round((fundSol * walletCount + 0.2) * LAMPORTS_PER_SOL));
  if (BigInt(creatorBal) < needed) {
    console.error(`creator needs at least ${(Number(needed) / LAMPORTS_PER_SOL).toFixed(2)} SOL; fund it at faucet.solana.com`);
    process.exit(1);
  }
  ...
  console.log(`migration flags supported by IDL: ${seq.migrationArgsSupported}`);
  ...
  for (const c of pre.buyChunks) {
    console.log(`  buy chunk (${c.walletCount}): ${c.result.unitsConsumed} CU, ok`);
  }
  ...
  check("launch txs landed (fund + create + buys)", sent.length >= 2 + Math.ceil(walletCount / 4), `${sent.length} txs`);
  ...
  console.log(`curve ${EXPLORER}/address/${seq.pda.curveState.toBase58()}?cluster=devnet`);
  ...
  check("every dev wallet holds tokens after launch", balAfterLaunch.every((b) => b > 0));

  const gate1 = await auto.readAutoCurveState(program, mint);
  check("curve state readable + NOT graduated", gate1.kind === "ok" && gate1.curve.graduated === false);
  ...
  console.log(`round result: completed=${buyRound.completed} failed=${buyRound.failed} skipped=${buyRound.skipped}`);
  check("auto buy round completed every wallet", buyRound.completed === walletCount && buyRound.failed === 0);
  ...
  check("holdings grew after the auto buy round", buyBalances.every((b) => b > 0));
  ...
  console.log(`round result: completed=${sellRound.completed} failed=${sellRound.failed} skipped=${sellRound.skipped}`);
  check("auto sell round completed every wallet", sellRound.completed === walletCount && sellRound.failed === 0);
  ...
  for (const d of devs) {
    ...
    console.log(`  ${d.publicKey.toBase58().slice(0, 12)}  tokens=${t}  sol=${(sol / LAMPORTS_PER_SOL).toFixed(4)}`);
  }
  check("auto sell drained every wallet to 0 tokens", soldOut.every((b) => b === 0));
  ...
  console.log(`round result: completed=${rebuy.completed} failed=${rebuy.failed} skipped=${rebuy.skipped}`);
  ...
  console.log(`route=${report.route} graduated=${report.graduated} sold=${report.sold} failed=${report.failed} skipped=${report.skipped} total=${report.total}`);
  for (const o of report.outcomes) {
    console.log(`  ${o.address.slice(0, 12)}  ${o.status}${o.reason ? ` (${o.reason.slice(0, 90)})` : ""}${o.signature ? `  ${EXPLORER}/tx/${o.signature}?cluster=devnet` : ""}`);
  }
  check("sell-all sold every keyed wallet (curve route)", report.route === "curve" && report.sold === walletCount && report.failed === 0);
  ...
  check("every wallet at 0 tokens after sell-all", finalToks.every((b) => b === 0));
  console.log(`holders after sell-all (devnet read): ${report.holderCountAfter}`);

  const gateFinal = await auto.readAutoCurveState(program, mint);
  check("curve still open + NOT graduated throughout (no accidental fill)", gateFinal.kind === "ok" && gateFinal.curve.graduated === false);

  console.log("\n===== devnet rehearsal done =====");
  console.log(`mint=${mint.toBase58()} curve=${seq.pda.curveState.toBase58()}`);
