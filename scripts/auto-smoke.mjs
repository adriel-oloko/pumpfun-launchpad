...
async function main() {
  const keypairPath = process.env.SOLANA_KEYPAIR ?? path.join(os.homedir(), ".config", "solana", "devnet.json");
  ...
  const provider = new anchorPkg.AnchorProvider(connection, new anchorPkg.Wallet(creator), {
    commitment: "confirmed",
  });
  const idl = JSON.parse(fs.readFileSync(path.join(repoRoot, "target/idl/pumpfun.json"), "utf8"));
  const program = new anchorPkg.Program(idl, provider);

  const walletData = JSON.parse(fs.readFileSync(path.join(__dirname, "dev-wallets.json"), "utf8"));
  ...
  console.log(`creator ${creator.publicKey.toBase58()} balance ${(creatorBal / LAMPORTS_PER_SOL).toFixed(4)} SOL`);
  if (creatorBal < BigInt(Math.round(0.1 * LAMPORTS_PER_SOL) * 2)) {
    console.error("creator balance too low to fund + create; fund it first (faucet.solana.com)");
    process.exit(1);
  }
  ...
  for (const d of devs) {
    fundTx.add(SystemProgram.transfer({ fromPubkey: creator.publicKey, toPubkey: d.kp.publicKey, lamports: fundLamports }));
  }
  fundTx.sign(creator);
  const fundSig = await connection.sendRawTransaction(fundTx.serialize());
  await connection.confirmTransaction(fundSig, "confirmed");
  console.log(`funded ${devs.length} wallets x ${fundSol} SOL: ${EXPLORER}/tx/${fundSig}?cluster=devnet`);

  for (const d of devs) {
    const b = await connection.getBalance(d.kp.publicKey);
    check(`wallet ${d.kp.publicKey.toBase58().slice(0, 12)} funded`, b >= BigInt(fundLamports), `${(b / LAMPORTS_PER_SOL).toFixed(4)} SOL`);
  }
  ...
  const createIx = await lib.buildCreateIx(program, creator.publicKey, pda, nonce, "M5 Auto Smoke", "M5AS", "https://example.com/m5-auto-smoke.json", { autoMigrate: false, lockLp: false });
  ...
  console.log(`created mint ${mintStr} (${EXPLORER}/address/${mintStr}?cluster=devnet)`);
  console.log(`curve ${pda.curveState.toBase58()} (${EXPLORER}/address/${pda.curveState.toBase58()}?cluster=devnet)`);

  const gate = await auto.readAutoCurveState(program, pda.mint);
  check("curve state readable", gate.kind === "ok", gate.kind === "ok" ? `creator ${gate.curve.creator.slice(0, 12)} graduated=${gate.curve.graduated}` : "missing");
  if (gate.kind !== "ok") process.exit(1);
  const curve = gate.curve;
  check("curve NOT graduated at start", curve.graduated === false);

  const mintBal = async (d) => lib.walletTokenBalance(connection, d.kp.publicKey, pda.mint);
  const solBal = async (d) => connection.getBalance(d.kp.publicKey);
  const walletList = devs.map((d) => ({ address: d.kp.publicKey.toBase58(), key: d.key }));
  ...
  check("buy round completed 3", buyRes.completed === 3, `completed=${buyRes.completed} failed=${buyRes.failed} skipped=${buyRes.skipped}`);
  ...
  for (const d of devs) {
    ...
    console.log(`  ${d.kp.publicKey.toBase58().slice(0, 12)}  tokens=${t}  sol=${(s / LAMPORTS_PER_SOL).toFixed(4)}`);
    check(`wallet ${d.kp.publicKey.toBase58().slice(0, 12)} holds tokens after buy`, t > BigInt(0));
  }
  ...
  check("sell round completed 3", sellRes.completed === 3, `completed=${sellRes.completed} failed=${sellRes.failed} skipped=${sellRes.skipped}`);

  for (const d of devs) {
    const t = await mintBal(d);
    const s = await solBal(d);
    console.log(`  ${d.kp.publicKey.toBase58().slice(0, 12)}  tokens=${t}  sol=${(s / LAMPORTS_PER_SOL).toFixed(4)}`);
    check(`wallet ${d.kp.publicKey.toBase58().slice(0, 12)} sold out (0 tokens)`, t === BigInt(0));
  }
  ...
  check("MIN SOL gate skipped all (0 completed)", gateBuy.completed === 0 && gateBuy.skipped === 3, `completed=${gateBuy.completed} skipped=${gateBuy.skipped}`);
  ...
  console.log(`top wallet holding after re-buy: ${holdRaw} raw (total supply ${params.TOTAL_SUPPLY.toString()})`);
  ...
  check("MIN % gate skipped all (holdings below 80% of supply)", gateSell.completed === 0, `completed=${gateSell.completed} skipped=${gateSell.skipped} minRaw=${minRaw80}`);

  console.log("\n===== auto-engine smoke done =====");
  console.log(`mint=${mintStr} curve=${pda.curveState.toBase58()}`);
  console.log(`wallets hold tokens (for the browser AUTO demo): ${(await mintBal(devs[0])) > BigInt(0) ? "yes" : "no"}`);
