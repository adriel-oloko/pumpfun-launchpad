...
export function Select(props: SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <select {...props} className={`input-brutal ${props.className ?? ""}`} />
  );
}
  ...
  return (
    <div>
      <div className="flex w-full justify-between gap-2">
        <Label>{label}</Label>
        {aside ? <span className="label-mono block opacity-60">{aside}</span> : null}
      </div>
  ...
  return (
    <a
      ...
      className={`underline underline-offset-2 ${
        inverted ? "hover:bg-paper hover:text-ink" : "hover:bg-ink hover:text-paper"
      }`}
    >
      {shortAddress(hash)}
    </a>
...
export function Collapse({
  ...
  return (
    <div>
      ...
      {open ? <div className="mt-3 flex flex-col gap-3 lg:grid lg:grid-cols-4">{children}</div> : null}
    </div>
  );
}
