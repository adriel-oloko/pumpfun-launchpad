...
export function TradePanel({
	...
	// DURATION, Auto Sell: SELL % / WALLET COUNT / DURATION). Count + duration
	// prefill v4's defaults; MIN SOL stays blank = no gate, and SELL % defaults
	// to 100 = sell each wallet's whole bag.
	...
	const [autoSellPct, setAutoSellPct] = useState("100");
	...
	// the effect body).
	const [tokenHeader, setTokenHeader] = useState<TokenHeaderInfo | null>(
		null,
	);
	const tokenMeta =
	...
	// >= MIN SOL; sell: token balance > 0) and
	...
	useEffect(() => {
		autoCfgRef.current = {
			...
			sellPct: parseAutoSellPct(autoSellPct),
		};
	});
          ...
          JSON at the on-chain uri (nothing renders while none exist). */}
            ...
            expiry) with checkbox-batch selection lives below the tabs. */}
              ...
              buy/sell omission back in) ---- */}
                ...
                label + MIN SOL / SELL % and wallet count + duration inputs side by
								...
								<div className="flex flex-col gap-2">
									...
									<div className="flex items-center gap-2 border-2 border-[#3a4956] px-2 py-1.5 rounded-md">
										...
										<Input
											...
											title="Sell this % of each wallet's own token holdings (blank/100 = all)"
		...
		return (
			<div className="reveal-up flex flex-col gap-1 border-2 border-ink px-2 py-1.5">
				<p className="label-mono !text-[11px] font-bold break-all">
					...
					{sent}/{outcomes.length} TO {shortAddress(dest, 6)} ·
