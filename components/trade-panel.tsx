...
export function TradePanel({
  ...
  const runManualTrade = async (side: "buy" | "sell") => {
    ...
    try {
      ...
      setManualError(msg);
      pushToast({
        action: side === "buy" ? "MANUAL BUY FAILED" : "MANUAL SELL FAILED",
        amount: "TX REVERTED",
        tone: "error",
      });
    } finally {
  ...
  const handleDisperse = async () => {
    ...
    if (!hub) {
      setDistributeError("DISPERSER FAILED: NO HUB (IMPORT A WALLET FIRST)");
      return;
    }
    if (!hub.key) {
      setDistributeError(
    ...
    if (min === null || max === null || min > max) {
      setDistributeError("DISPERSER FAILED: INVALID MIN/MAX (0 <= MIN <= MAX)");
      return;
    }
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      const firstSig = outcomes.find((o) => o.signature)?.signature ?? null;
      if (sent > 0) {
        pushToast({
          action: "WITHDREW",
  ...
  const handleDeleteSelected = () => {
    ...
    if (toDelete.length === 0) {
      setDistributeError(null);
      setDistributeReport({ kind: "delete", removed: 0, skipped, deleted: [] });
      pushToast({
        action: "DELETE: NOTHING EMPTY",
        amount: skipped > 0 ? `${skipped} SKIPPED (FUNDED / UNKNOWN)` : undefined,
        tone: "error",
      });
      return;
    }
    ...
    try {
      ...
      setDistributeReport({ kind: "delete", removed: toDelete.length, skipped, deleted: toDelete });
      pushToast({
        action: "DELETED",
  ...
  async function autoBuyTick() {
    ...
    void fireAutoBuy({
      ...
      .catch((e) => {
        ...
        if (autoRunningRef.current) {
          const raw = e instanceof Error ? e.message : String(e);
          setAutoStatus(`AUTO BUY ROUND ERROR: ${friendlyTxError(raw)} (RETRYING NEXT ROUND)`);
        }
      })
      .finally(() => {
  ...
  async function autoSellTick() {
    ...
    void fireAutoSell({
      ...
      .catch((e) => {
        ...
        if (autoRunningRef.current) {
          const raw = e instanceof Error ? e.message : String(e);
          setAutoStatus(`AUTO SELL ROUND ERROR: ${friendlyTxError(raw)} (RETRYING NEXT ROUND)`);
        }
      })
      .finally(() => {
        autoLockRef.current = false;
  ...
  const handleStartAuto = async () => {
    ...
    setAutoRunning(true);
    const parts: string[] = [];
    if (cfg.buyOn) parts.push(`BUY EVERY ${Math.round(cfg.buyDurationMs / 1000)}S`);
    if (cfg.sellOn) parts.push(`SELL EVERY ${Math.round(cfg.sellDurationMs / 1000)}S`);
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      ...
      setAutoBuyLeft(
        autoBuyEndRef.current === null
          ? null
          : Math.max(0, Math.ceil((autoBuyEndRef.current - now) / 1000))
      );
      setAutoSellLeft(
        autoSellEndRef.current === null
          ? null
          : Math.max(0, Math.ceil((autoSellEndRef.current - now) / 1000))
      );
    }, 250);
        ...
        <div className="sticky top-0 z-30 flex flex-col gap-4 bg-paper">
          ...
          <div className="border-t-2 border-ink pt-3">
            ...
            <div
              ...
              <label role="tab" aria-label="Buy / Sell" className="tab">
                <input
                  type="radio"
                  name="managed-tabs"
                  className="sr-only"
              ...
              <div role="tabpanel" className="tab-content">
                ...
                <div className="mt-2 grid gap-2 md:grid-cols-5 [auto_1fr_1fr]">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    />
                    <span className="label-mono text-[10px] opacity-60">%</span>
                  </div>
                  <button
                    type="button"
                    className="btn-brutal h-full shadow-none!"
                    onClick={() => void runManualTrade("buy")}
                    disabled={
                      manualBusy ||
                      autoRunning ||
                      !mint ||
                  ...
                  <button
                    ...
                    onClick={() => void runManualTrade("sell")}
                    disabled={
                      manualBusy ||
                      autoRunning ||
                      !mint ||
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-2 border-2 border-ink px-2 py-1.5">
                    <input
                      ...
                      checked={autoBuyOn}
                      onChange={(e) => setAutoBuyOn(e.target.checked)}
                    ...
                    <Input
                      ...
                      step="0.001"
                      value={autoBuyMinSol}
                      onChange={(e) => setAutoBuyMinSol(e.target.value)}
                      placeholder="MIN SOL"
                    ...
                    <Input
                      ...
                      onChange={(e) => setAutoBuyWallets(e.target.value)}
                    ...
                    <Input
                      ...
                      onChange={(e) => setAutoBuyDuration(e.target.value)}
                    ...
                    <span className="label-mono !text-[10px] shrink-0 w-9 text-right tabular-nums">
                      {autoBuyOn && autoRunning && autoBuyLeft !== null
                        ? `${autoBuyLeft}s`
                        : ""}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 border-2 border-ink px-2 py-1.5">
                    <input
                      ...
                      className="checkbox-brutal shrink-0"
                      checked={autoSellOn}
                      onChange={(e) => setAutoSellOn(e.target.checked)}
                    ...
                    <Input
                      ...
                      onChange={(e) => setAutoSellMinPct(e.target.value)}
                      placeholder="%"
                    ...
                    <Input
                      ...
                      onChange={(e) => setAutoSellWallets(e.target.value)}
                      placeholder="WALLET COUNT"
                    ...
                    <Input
                      ...
                      onChange={(e) => setAutoSellDuration(e.target.value)}
                      placeholder="DURATION"
                    ...
                    />
                    <span className="label-mono !text-[10px] shrink-0 w-9 text-right tabular-nums">
                      {autoSellOn && autoRunning && autoSellLeft !== null
                        ? `${autoSellLeft}s`
                        : ""}
                    </span>
                  </div>
                  ...
                  {autoStatus ? (
                    <StatusLine text={autoStatus} tone="idle" />
                  ) : null}
                </div>
              </div>
              ...
              <div role="tabpanel" className="tab-content">
                ...
                <div className="flex w-full gap-2">
                  <Input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") api.handleImport();
                    }}
                  ...
                  <Input
                    ...
                    onChange={(e) => api.setRandomCount(e.target.value)}
                    placeholder="N"
                ...
                <div className="mt-2 grid grid-cols-2 gap-2 md:grid-cols-5">
                  <Input
                    ...
                    step="0.001"
                    value={disperseMin}
                    onChange={(e) => setDisperseMin(e.target.value)}
                    placeholder="MIN"
                  ...
                  <Input
                    ...
                    step="0.001"
                    value={disperseMax}
                    onChange={(e) => setDisperseMax(e.target.value)}
                    placeholder="MAX"
                  ...
                  <Btn
                    ...
                    Disperse {disperseMin || "0"}–{disperseMax || "0"} SOL
                  </Btn>
                  <Btn
                    invert
                ...
                {distributeError ? (
                  <StatusLine text={distributeError} tone="error" />
                ) : null}
                {distributeReport ? (
                  <DistributeReportView report={distributeReport} />
                ) : null}
                {hub ? (
                  <p className="label-mono text-[10px] opacity-60 mt-1">
                    VIA {shortAddress(hub.address, 6)} — DISPERSER · WITHDRAW DEFAULT DEST
                    {hub.key ? "" : " · NO KEY — RE-ADD TO DISPERSE"}
                  </p>
                ) : null}
              </div>
            </div>
        ...
        {withdrawOpen ? (
          <div
            ...
            <div
              ...
              <div className="mt-3">
                <label className="label-mono block !text-[11px]">To</label>
                <Input
                  type="text"
                  autoComplete="off"
                  spellCheck={false}
              ...
              <div className="mt-3 flex gap-2">
                ...
                <Btn
                  ...
                  disabled={withdrawBusy || selectedKeyedWallets.length === 0}>
...
function ManualReportView({ report }: { report: ManualBatchReport }) {
  ...
  return (
    <div className="flex flex-col gap-1 border-2 border-ink px-2 py-1.5">
      <p className="label-mono !text-[11px] font-bold">
        {side === "buy" ? "BUY" : "SELL"} {pct}%: {verb} {result.completed}/
        {total} · SKIPPED {result.skipped} · FAILED {result.failed}
      </p>
      {result.signatures.length > 0 ? (
        <div className="flex flex-col gap-0.5 border-t border-ink/40 pt-1">
          {result.signatures.map((sig) => (
            <p key={sig} className="label-mono !text-[10px] break-all opacity-90">
...
function DistributeReportView({ report }: { report: DistributeReport }) {
    ...
    return (
      <div className="flex flex-col gap-1 border-2 border-ink px-2 py-1.5">
        <p className="label-mono !text-[11px] font-bold break-all">
          DISPERSED {result.count} WALLET{result.count === 1 ? "" : "S"} ·{" "}
  ...
  return (
    <div className="flex flex-col gap-1 border-2 border-ink px-2 py-1.5">
      <p className="label-mono !text-[11px] font-bold">
        DELETE: REMOVED {removed} · SKIPPED {skipped} (NOT EMPTY / UNKNOWN
        BALANCE)
      </p>
      {deleted.length > 0 ? (
        <div className="flex flex-col gap-0.5 border-t border-ink/40 pt-1">
