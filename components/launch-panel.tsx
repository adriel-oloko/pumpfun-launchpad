...
import { Btn, Card, Collapse, ExplorerLink, Field, Input, StatusLine } from "./ui";
...
export function LaunchPanel({
  ...
  const { key: creatorKey, pubkey: creatorPubkey, connected, balanceSol } =
    useCreatorWallet();
  ...
  const parseSolIn = (raw: string | undefined): bigint => {
    ...
    if (!Number.isFinite(n) || n <= 0) {
      throw new Error(`sol_in must be a positive number, got "${raw ?? ""}"`);
    }
  ...
  const handleLaunch = async () => {
    ...
    try {
      ...
      if (Buffer.byteLength(name, "utf8") > 32) throw new Error(`name too long (${Buffer.byteLength(name, "utf8")} > 32 bytes)`);
      if (Buffer.byteLength(symbol, "utf8") > 10) throw new Error(`symbol too long (${Buffer.byteLength(symbol, "utf8")} > 10 bytes)`);
      if (Buffer.byteLength(uri, "utf8") > 200) throw new Error(`uri too long (${Buffer.byteLength(uri, "utf8")} > 200 bytes)`);
      ...
      for (const w of selectedWallets) {
        if (!w.key) {
          throw new Error(`wallet ${shortAddress(w.address, 6)} has no key (watch-only wallets cannot sign buys)`);
        }
        const rawSolIn = solIns[w.address];
        const solIn = parseSolIn(
          rawSolIn === undefined || rawSolIn.trim() === "" ? DEFAULT_SOL_IN : rawSolIn
        );
        buys.push({
      ...
      log(`uri     : ${uri}`);
      for (const b of buys) {
        log(`  dev ${b.wallet.publicKey.toBase58().slice(0, 12)}... buys ${(Number(b.solInLamports) / LAMPORTS_PER_SOL).toFixed(4)} SOL`);
      }
      log(`migrate : auto_migrate=${autoMigrate} lock_lp=${lockLp} (idl ${capability.autoMigrate && capability.lockLp ? "supports" : "PRE-M3, not supported"})`);
      ...
      log(`nonce   : ${nonce}`);
      log(`mint    : ${EXPLORER}/address/${PUMPFUN_PROGRAM_ID.toBase58()} (program) — pda below`);
      ...
      const fundLamportsPerWallet = fundFromCreator
        ? buys.map(
            (b) => b.solInLamports + BigInt(ataRent) + postBuyFloorLamports()
          )
        : null;
      if (fundLamportsPerWallet) {
        const totalFund = fundLamportsPerWallet.reduce((a, b) => a + b, BigInt(0));
        const creatorBal = await connection.getBalance(creator.publicKey, "confirmed");
        const needed = totalFund + BigInt(Math.round(0.02 * LAMPORTS_PER_SOL));
        log(`fund    : ${(Number(totalFund) / LAMPORTS_PER_SOL).toFixed(4)} SOL from creator (sol_in + ata rent + floor)`);
      ...
      for (const c of pre.buyChunks) {
        log(`   buy${c.buyTxIndex + 1} chunk (${c.walletCount}): ${c.result.unitsConsumed} CU, ok`);
      }
      ...
      if (tier === "1") {
        ...
        const sent = await sendSequentially(connection, seq, {
          onSignature: (label, sig) => {
            sentSigs.push(sig);
            log(`[${label}] ${sig}  ${EXPLORER}/tx/${sig}?cluster=devnet`);
          },
        });
        log(`sent ${sent.length} txs: ${sent.map((s) => s.label).join(", ")}`);
      } else {
        ...
        try {
          const tips = await withTimeout(jito.getTipAccounts(), 10_000, "getTipAccounts timed out");
          log(`jito endpoint reachable (${tips.length} tip accounts)`);
        } catch (e) {
          unreachable = errMsg(e);
          log(`jito endpoint ${JITO_DEVNET_ENDPOINT}: UNREACHABLE (${unreachable})`);
          log("   matches devnet reality: devnet.block-engine.jito.wtf does not resolve;");
          log("   devnet bundles cannot be submitted or land.");
        }
        const latest = await connection.getLatestBlockhash("confirmed");
        const tipAccount = new PublicKey(KNOWN_TIP_ACCOUNTS[0]);
        const bundleTxs = [seq.fundTx, seq.createTx, ...seq.buyTxs.map((b) => b.tx)].filter(
        ...
        log(`bundle assembled: ${assembled.base64.length} txs, tip ${MIN_TIP_LAMPORTS} lamports -> ${tipAccount.toBase58()}`);
        ...
        const sims = await simulateBundle(connection, {
          ...
          buyTxs: seq.buyTxs.map((bt) => ({ wallets: bt.wallets, instructions: bt.instructions })),
          tipIx,
          creator,
        });
        for (const s of sims) log(`   ${s.label}: ${s.unitsConsumed} CU, ok`);
        if (unreachable) {
          log("TIER 2 RESULT: bundle assembled + simulated; submission impossible on devnet");
          log("   (block engine host does not resolve). No fabricated landing claim.");
        } else {
          log("submitting bundle via relay fan-out (jito primary + bloxroute + astralane, same-origin proxy)...");
          ...
          log(`TIER 2 RESULT: ${tier2Result.outcome}${tier2Result.bundleId ? ` (bundle ${tier2Result.bundleId})` : ""}${tier2Result.landedSlot != null ? `, landed slot ${tier2Result.landedSlot}` : ""}`);
        }
        ...
        if (!tier2Result || tier2Result.outcome !== "landed") {
          throw new Error(
            tier2Result ? bundleDropMessage(tier2Result) : "TIER 2 BUNDLE HAD NO RESULT: nothing was created."
          );
        }
      }
      ...
      log(`curve   : ${EXPLORER}/address/${seq.pda.curveState.toBase58()}?cluster=devnet`);

      const balances = [];
      for (const b of buys) {
        const bal = await walletTokenBalance(connection, b.wallet.publicKey, mintPk);
        balances.push(bal);
        log(
      ...
      try {
        ...
        log("holders (getTokenLargestAccounts): rate-limited on the public devnet RPC;");
        log("   the selected-wallet roster above is the authoritative count.");
      }
      try {
      ...
      if (meta) {
        log(`metadata  : name="${meta.name}" symbol="${meta.symbol}" uri=${meta.uri}`);
      } else {
      ...
      if (rawMsg.includes("InvalidProgramId") || rawMsg.includes("Custom\":3008")) {
        log(
      ...
      const bundleDrop = /BUNDLE DID NOT LAND|BUNDLE COULD NOT BE SUBMITTED|JITO BUNDLE|TIER 2 BUNDLE/i.test(
        rawMsg
      );
      pushToast({
    ...
    <Card
      ...
      <div className="flex flex-col gap-4">
        ...
        <Collapse
            ...
            <div className="flex flex-wrap items-center gap-4 border-2 border-ink px-3 py-2">
              ...
              <label className="label-mono flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  checked={lockLp}
                  disabled={!migrationSupported}
                  onChange={(e) => setLockLp(e.target.checked)}
                />
                lp-lock
              </label>
          ...
          <div className="lg:col-span-4">
            <div className="flex flex-wrap items-center gap-4 border-2 border-ink px-3 py-2">
              <span className="label-mono opacity-70">send path</span>
              ...
              <label className="label-mono flex items-center gap-2 cursor-pointer opacity-80">
                <input
                  ...
                  onChange={(e) => setFundFromCreator(e.target.checked)}
                />
          ...
          <div className="min-w-full grid md:grid-cols-2 lg:grid-cols-3">
          {selectedWallets.map((w) => (
            <Field
              ...
              <Input
                ...
                onChange={(e) => setSolIn(w.address, e.target.value)}
