...
export interface RosterApi {
	...
	/** Copy EVERY roster address (HUB included) to the clipboard, one per
	 *  line: the exact shape the Deposit window's paste cascade splits on
	 *  (`[\s,]+`). Pushes an `N ADDRESSES COPIED` toast; a clipboard
	 *  rejection pushes COPY FAILED rather than failing silently. No-op on an
	 *  empty roster (the button is not rendered then either). */
...
export function useRoster(): RosterApi {
	...
	const hubAddress =
		wallets.length > 0 ? (wallets[0]?.address ?? null) : null;
	...
	const tick = useCallback(async () => {
		...
		try {
			...
			addresses.forEach((a, i) => {
				...
				solMap.set(
					a,
					infos[i] ? BigInt(infos[i]!.lamports) : BigInt(0),
				);
			});
		} catch {
	...
	/** Act-column header button: copy every roster address, HUB included,
	 *  newline-separated. No transient row highlight (there is no single row
	 *  to mark): the toast is the whole feedback channel, like KEY COPIED. */
	const copyAllAddresses = () => {
		const addrs = wallets.map((w) => w.address);
		if (addrs.length === 0) return;
		navigator.clipboard
			.writeText(addrs.join("\n"))
			.then(() =>
				pushToast({
					action: `${addrs.length} ADDRESSES COPIED`,
					tone: "ok",
				}),
			)
			.catch(() => pushToast({ action: "COPY FAILED", tone: "error" }));
	};
		...
		<div>
			...
			<div className="label-mono flex items-center gap-2 border-2 border-[#3a4956] bg-[#52d593] px-3 py-1.5 text-ink rounded-t-md">
				...
				<span className="flex w-16 shrink-0 items-center justify-end gap-1">
					<span>Act</span>
					{/* Copy-all: every roster address (HUB included), newline
					    separated, so the whole set pastes as a list. */}
					<button
						type="button"
						title="Copy all addresses (HUB included, one per line)"
						aria-label="copy all wallet addresses"
						className="p-0.5 opacity-70 transition-opacity duration-150 hover:opacity-100"
						onClick={api.copyAllAddresses}>
						<svg
							width="13"
							height="13"
							viewBox="0 0 24 24"
